...
public class Parser {
	...
	public static void run(ArrayList<String> inputlist){
		...
		for(int i = 0 ; i <inputlist.size(); i++ ){
			if(inputlist.get(i).length()==2){
				command =isCommand(inputlist.get(i));
			}
...
//column
	...
	public static int isCommand(String testThis){
		...
		if(testThis.equals("-e"))
			return 12;
		if((testThis.substring(0, 1).equals("-"))){
			String error = testThis+ " is not a command";
			errorlog.add(error);
